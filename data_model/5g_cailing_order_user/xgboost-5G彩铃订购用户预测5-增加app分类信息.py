...
from utils.featureProjectUtils import getFeatureNoneRate, getSiPiEeMan
...
def getModelResult(trainFilePath,testFilePath,labels):
    ...
    feature_params = ['prov_id', 'area_id', 'innet_months', 'service_type', 'cust_sex', 'cert_age', 'total_fee','jf_flux', 'fj_arpu', 'ct_voice_fee', 'total_flux', 'total_dura', 'roam_dura',
    ...
    result_feature = ['prov_id', 'user_id', 'cust_id', 'product_id', 'area_id', 'device_number', 'innet_months','service_type', 'cust_sex', 'cert_age',
                      'total_fee', 'jf_flux', 'fj_arpu', 'ct_voice_fee', 'total_flux', 'total_dura', 'brand_detail', 'price', 'imei_duration', 'avg_duratioin',
                      'app_type_id', 'app_active_days', 'app_visit_dura']
    ...
    # 训练模型
    y_test = ''
    print('开始训练模型数据 - ', strftime("%Y-%m-%d %H:%M:%S", localtime()))
...
if __name__ == '__main__':
    ...
    paths = [test_path2,test_path4,test_path5]
    labels = []
    for path in paths:
